...
class AdminRepository:
    ...
    async def update_admin(self, db: AsyncSession, admin: int, db_admin_data: Admin) -> Admin:
    ...
    async def modify_user_active_status(self, db: AsyncSession, user_id: int, active: bool) -> Users:
    ...
    async def update_product_active_status(self, db: AsyncSession, product_id: int, active: bool) -> Product:
        result = await db.execute(select(Product).filter(Product.Id_Product == product_id))
