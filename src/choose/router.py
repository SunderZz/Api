...
async def get_chooseoses(choose_repository: ChooseRepository = Depends(ChooseRepository), db:AsyncSession = Depends(get_db)) -> list[ChooseBase]:
...
@router.get("/choose/{choose_id}", status_code=status.HTTP_200_OK, response_model=ChooseBase)
async def get_chooseose_by_id(choose_id: int, choose_repository: ChooseRepository = Depends(ChooseRepository), db:AsyncSession = Depends(get_db)) -> ChooseBase:
...
async def create_choose(choose: ChooseBase, choose_repository: ChooseRepository = Depends(ChooseRepository), db:AsyncSession = Depends(get_db)) -> ChooseBase:
...
@router.put("/choose/{choose_id}", status_code=status.HTTP_200_OK, response_model=ChooseBase)
async def update_choose(choose_id: int, choose: ChooseBase, choose_repository: ChooseRepository = Depends(ChooseRepository), db:AsyncSession = Depends(get_db)) -> ChooseBase:
