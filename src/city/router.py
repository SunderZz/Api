...
async def get_cities(city_repository: CityRepository = Depends(CityRepository),db:AsyncSession = Depends(get_db))-> list[CityIdBase]:
...
async def get_city_id(city_repository: CityRepository = Depends(CityRepository),db:AsyncSession = Depends(get_db))-> list[CityIdBase]:
    cities = await city_repository.get_city(db)
    city_list = [model_to_dict(city) for city in cities]
    return [CityIdBase(**city_dict) for city_dict in city_list]
...
async def get_city_by_names(city: str, city_repository: CityRepository = Depends(CityRepository), db:AsyncSession = Depends(get_db)) -> CityIdBase:
...
async def get_city_with_ids(city: int, city_repository: CityRepository = Depends(CityRepository), db:AsyncSession = Depends(get_db)) -> CityBase:
