...
async def get_cities_service(city_repository: CityRepository, db: AsyncSession) -> list[CityIdBase]:
...
async def get_city_by_name_service(city: str, city_repository: CityRepository, db: AsyncSession) -> CityIdBase:
...
async def get_city_by_id_service(city: int, city_repository: CityRepository, db: AsyncSession) -> CityBase:
    value = await city_repository.get_city_by_id(db, city)
    if value is None:
        raise HTTPException(status_code=404, detail="city not found or attribute not found")
    return CityBase(**model_to_dict(value))

async def create_city_service(city: CityBase, city_repository: CityRepository, db: AsyncSession) -> CityIdBase:
...
async def update_city_service(city_id: int, city: CityBase, city_repository: CityRepository, db: AsyncSession) -> CityBase:
