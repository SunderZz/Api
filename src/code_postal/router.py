...
async def get_code_postal_value(
    code_postal: int,
    code_postal_repository: CodePostalRepository = Depends(CodePostalRepository),
    db: AsyncSession = Depends(get_db),
) -> CodePostalIdBase:
...
async def create_code_postales(
    ...
    existing_code_postal = await get_code_postal_name(
        code_postal.code_postal, code_postal_repository, db
    )
...
async def update_code_postal(
    ...
    if not existing_code_postal:
        code_postal_created = await create_code_postales(
            code_postal, code_postal_repository, db
        )
