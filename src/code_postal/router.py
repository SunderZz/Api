...
async def get_code_postal_name(code_postal: int, code_postal_repository: CodePostalRepository = Depends(CodePostalRepository), db:AsyncSession= Depends(get_db)) -> CodePostalBase:
    value = await code_postal_repository.get_code_postal_query(db, code_postal)
    if value is None:
        return None
    code_postal_dict = model_to_dict(value)
...
async def create_code_postales(code_postal: CodePostalBase,code_postal_repository: CodePostalRepository = Depends(CodePostalRepository), db:AsyncSession= Depends(get_db))-> CodePostalBase:
...
async def update_code_postal(code_postal_id: int, code_postal: CodePostalBase,code_postal_repository: CodePostalRepository = Depends(CodePostalRepository), db:AsyncSession= Depends(get_db))-> CodePostalBase:
