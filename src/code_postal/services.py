...
async def get_code_postal_service(
    code_postal_repository: CodePostalRepository, db: AsyncSession
) -> list[CodePostalBase]:
...
async def create_code_postales_service(
    code_postal: CodePostalBase,
    code_postal_repository: CodePostalRepository,
    db: AsyncSession,
) -> CodePostalIdBase:
    existing_code_postal = await code_postal_repository.get_code_postal_query(
        db, code_postal.code_postal
    )
...
async def update_code_postal_service(
    code_postal_id: int,
    code_postal: CodePostalBase,
    code_postal_repository: CodePostalRepository,
    db: AsyncSession,
) -> CodePostalIdBase:
    updated_code_postal = await code_postal_repository.update_code_postal(
        db, code_postal_id, code_postal
    )
...
async def get_city_by_code_service(
    code_id: int,
