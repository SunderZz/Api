...
async def get_customers_service(customers_repository: CustomersRepository, db: AsyncSession) -> list[CustomersBase]:
...
async def get_customer_value_service(customers: int, customers_repository: CustomersRepository, db: AsyncSession) -> CustomersBase:
...
async def create_customer_service(customer: CustomersBase, customers_repository: CustomersRepository, db: AsyncSession) -> CustomersBase:
    new_customer = await customers_repository.create_customers(db, customer)
    return CustomersBase(**model_to_dict(new_customer))

async def update_customer_service(customer_id: int, customer: CustomersBase, customers_repository: CustomersRepository, db: AsyncSession) -> CustomersBase:
    updated_customer = await customers_repository.update_customers(db, customer_id, customer)
