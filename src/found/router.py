...
async def get_founds(found_repository: FoundRepository = Depends(FoundRepository), db: Session = Depends(get_db)) -> list[FoundBase]:
...
async def get_found_by_id(found_id: int, found_repository: FoundRepository = Depends(FoundRepository), db: Session = Depends(get_db)) -> FoundBase:
...
async def create_found(found: FoundBase, found_repository: FoundRepository = Depends(FoundRepository), db: Session = Depends(get_db)) -> FoundBase:
...
async def update_found(found_id: int, found: FoundBase, found_repository: FoundRepository = Depends(FoundRepository), db: Session = Depends(get_db)) -> FoundBase:
