...
async def get_founds_service(
    found_repository: FoundRepository, db: AsyncSession
) -> list[FoundBase]:
...
async def get_found_by_id_service(
    found_id: int, found_repository: FoundRepository, db: AsyncSession
) -> FoundBase:
...
async def create_found_service(
    found: FoundBase, found_repository: FoundRepository, db: AsyncSession
) -> FoundBase:
    new_found = await found_repository.create_found(db, found)
    return FoundBase(**model_to_dict(new_found))


async def update_found_service(
    found_id: int, found: FoundBase, found_repository: FoundRepository, db: AsyncSession
) -> FoundBase:
