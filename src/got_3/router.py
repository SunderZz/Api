...
async def get_gots(got_repository: GotRepository = Depends(GotRepository), db:AsyncSession = Depends(get_db)) -> list[GotBase]:
...
async def get_got_by_id(got_id: int, got_repository: GotRepository = Depends(GotRepository), db:AsyncSession = Depends(get_db)) -> GotBase | list[GotBase]:
...
async def update_got( got: GotBase,got_id: int| None = None, got_repository: GotRepository = Depends(GotRepository), db:AsyncSession = Depends(get_db)) -> GotBase:
