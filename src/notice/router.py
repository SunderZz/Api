...
@router.get("/notice/", status_code=status.HTTP_200_OK, response_model=list[NoticeBase] |NoticeBase |None)
async def get_notices_by_id(notice:int,notice_repository: NoticeRepository = Depends(NoticeRepository),db:AsyncSession = Depends(get_db))-> list[NoticeBase] |NoticeBase | None:
...
@router.post("/notice/", status_code=status.HTTP_201_CREATED, response_model=NoticeCreateBase)
async def create_notice(notice: NoticeBase,notice_repository: NoticeRepository = Depends(NoticeRepository), db:AsyncSession = Depends(get_db))-> NoticeCreateBase:
