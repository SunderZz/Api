...
async def get_operate(operate_repository: OperateRepository = Depends(OperateRepository), db: AsyncSession = Depends(get_db)) -> list[OperateBase]:
...
async def get_operates(operate_repository: OperateRepository = Depends(OperateRepository), db:AsyncSession = Depends(get_db)) -> list[OperateBase]:
    operates = await operate_repository.get_operate(db)
...
async def create_operate(operate: OperateBase,operate_id:int, operate_repository: OperateRepository = Depends(OperateRepository), db:AsyncSession = Depends(get_db)) -> OperateBase:
...
async def update_operate(operate_id: int, operate: OperateBase, operate_repository: OperateRepository = Depends(OperateRepository), db:AsyncSession = Depends(get_db)) -> OperateBase:
