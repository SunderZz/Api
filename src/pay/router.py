...
async def get_pays(pay_repository: PayRepository = Depends(PayRepository), db:AsyncSession = Depends(get_db)) -> list[PayBase]:
...
async def get_pay_by_id(payment_id: int, pay_repository: PayRepository = Depends(PayRepository), db:AsyncSession = Depends(get_db)) -> PayBase:
...
async def create_pay(pay: PayBase, pay_repository: PayRepository = Depends(PayRepository), db:AsyncSession = Depends(get_db)) -> PayBase:
...
async def update_pay(payment_id: int, pay: PayBase, pay_repository: PayRepository = Depends(PayRepository), db:AsyncSession = Depends(get_db)) -> PayBase:
