...
class PaymentRepository:
    ...
    async def get_payment_query(self, db: AsyncSession, payment: int) -> Payment:
        result = await db.execute(select(Payment).filter(Payment.Id_Payments == payment))
    ...
    async def update_payment(self, db: AsyncSession, payment_id: int, db_payment_data: Payment) -> Payment:
        result = await db.execute(select(Payment).filter(Payment.Id_Payments == payment_id))
