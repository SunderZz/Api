...
async def get_payments(payment_repository: PaymentRepository = Depends(PaymentRepository),db: Session = Depends(get_db))-> list[PaymentBase]:
...
async def get_payment_value(payment: str, payment_repository: PaymentRepository = Depends(PaymentRepository), db: Session = Depends(get_db)) -> PaymentBase:
...
async def create_payment(payment: PaymentBase,payment_repository: PaymentRepository = Depends(PaymentRepository), db: Session = Depends(get_db))-> PaymentBase:
...
async def update_payment(payment_id: int, payment: PaymentBase,payment_repository: PaymentRepository = Depends(PaymentRepository), db: Session = Depends(get_db))-> PaymentBase:
