...
class ProducersRepository:
    ...
    async def get_producers_query(self, db: AsyncSession, producers: int) -> Producers:
        result = await db.execute(
            select(Producers).filter(Producers.Id_Users == producers)
        )
        return result.scalar_one_or_none()

    async def get_user_query(self, db: AsyncSession, producers: int) -> Producers:
        result = await db.execute(
            select(Producers).filter(Producers.Id_Producers == producers)
        )
        return result.scalar_one_or_none()
    ...
    async def update_producers(
        self, db: AsyncSession, producers: int, db_producers_data: Producers
    ) -> Producers:
        result = await db.execute(
            select(Producers).filter(Producers.Id_Producers == producers)
        )
