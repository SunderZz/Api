...
class ProductRepository:
    ...
    async def get_product_id_by_name(self, db: AsyncSession, name: str) -> list[Product]:
    ...
    async def get_product_by_discount(self, db: AsyncSession) -> list[Product]:
        result = await db.execute(select(Product).filter(Product.Discount.isnot(None), Product.Discount > 0))
        return result.scalars().all()
    
    async def get_products_by_name(self, db: AsyncSession, product_name: str) -> Product | list[Product] | None:
    ...
    async def update_product(self, db: AsyncSession, product: int, db_product_data: Product) -> Product:
        result = await db.execute(select(Product).filter(Product.Id_Product == product))
