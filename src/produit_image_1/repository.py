...
class ProduitImageRepository:
    ...
    async def get_produit_image_query(self, db: AsyncSession, produit_image: int) -> Produit_Image:
        result = await db.execute(select(Produit_Image).filter(Produit_Image.Id_Produit_Image == produit_image))
    ...
    async def update_Produit_Image(self, db: AsyncSession, produit_image_id: int, db_produit_image_data: Produit_Image) -> Produit_Image:
        result = await db.execute(select(Produit_Image).filter(Produit_Image.Id_Produit_Image == produit_image_id))
