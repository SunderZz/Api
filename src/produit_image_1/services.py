...
async def get_produit_images_service(produit_image_repository: ProduitImageRepository, db: AsyncSession) -> list[ProduitImageBase]:
...
async def get_produit_image_value_service(produit_image: str, produit_image_repository: ProduitImageRepository, db: AsyncSession) -> ProduitImageBase:
...
async def create_produit_image_service(produit_image: ProduitImageBase, produit_image_repository: ProduitImageRepository, db: AsyncSession) -> ProduitImageBase:
...
async def update_produit_image_service(produit_image_id: int, produit_image: ProduitImageBase, produit_image_repository: ProduitImageRepository, db: AsyncSession) -> ProduitImageBase:
    unique_id = uuid.uuid4()
    lien_image = f"http://example.com/images/{produit_image.Nom}_{unique_id}.jpg"
