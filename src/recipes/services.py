...
async def search_recipes_service(query: str, recipes_repository: RecipesRepository, db: AsyncSession) -> list[RecipesBase] | RecipesBase | None:
...
async def get_recipes_service(recipes_repository: RecipesRepository, db: AsyncSession) -> list[RecipesBase]:
    recipes = await recipes_repository.get_Recipes(db)
    recipes_list = [model_to_dict(recipe) for recipe in recipes]
    return [RecipesBase(**recipes_dict) for recipes_dict in recipes_list]

async def get_recipes_value_service(product: str, recipe: str, recipes_repository: RecipesRepository, db: AsyncSession) -> list[RecipesBase] | RecipesBase | None:
...
async def get_recipes_by_products_service(recipe: int, recipes_repository: RecipesRepository, db: AsyncSession) -> RecipesBase:
...
async def create_recipes_service(recipes: RecipesCreateBase, products_repository: ProductRepository, found_repository: FoundRepository, recipes_repository: RecipesRepository, db: AsyncSession) -> RecipesBase:
...
async def update_recipes_service(recipes_id: int, recipes: RecipesCreateBase, recipes_repository: RecipesRepository, db: AsyncSession) -> RecipesBase:
