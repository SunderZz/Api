...
async def get_redacts(redact_repository: RedactRepository = Depends(RedactRepository), db:AsyncSession = Depends(get_db)) -> list[RedactBase]:
...
async def get_redact_value(admin_id:int, redact_id: int, redact_repository: RedactRepository = Depends(RedactRepository), db:AsyncSession = Depends(get_db)) -> RedactBase:
...
async def create_redact(admin_id:int,redact_id:int,redact: RedactBase, redact_repository: RedactRepository = Depends(RedactRepository), db:AsyncSession = Depends(get_db)) -> RedactBase:
...
async def update_redact(admin_id: int, recipe_id: int, redact: RedactBase, redact_repository: RedactRepository = Depends(RedactRepository), db:AsyncSession = Depends(get_db)) -> RedactBase:
