...
@router.get("/seasons", status_code=status.HTTP_200_OK, response_model=list[SeasonRetrieveBase])
async def get_seasons(season_repository: SeasonRepository = Depends(SeasonRepository),db:AsyncSession = Depends(get_db))-> list[SeasonRetrieveBase]:
...
@router.get("/get_seasons_by_id", status_code=status.HTTP_200_OK, response_model=SeasonRetrieveBase)
async def get_seasons_with_product_id(id: int,season_repository: SeasonRepository = Depends(SeasonRepository), db:AsyncSession = Depends(get_db)) -> SeasonRetrieveBase:
...
@router.get("/get_seasons_by_name", status_code=status.HTTP_200_OK, response_model=SeasonRetrieveBase)
async def get_seasons_by_name(name: str,season_repository: SeasonRepository = Depends(SeasonRepository), db:AsyncSession = Depends(get_db)) -> SeasonRetrieveBase:
