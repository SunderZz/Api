...
class ShipmentsCostRepository:
    ...
    async def get_one_shipment(
        ...
        result = await db.execute(
            select(Shipments_Cost).filter(
                Shipments_Cost.Id_Shipments_Cost == shipment_id
            )
        )
        return result.scalar_one_or_none()

    async def create_shipments_Cost(
        self, db: AsyncSession, shipments_cost: Shipments_Cost
    ) -> Shipments_Cost:
    ...
    async def update_Shipments_Cost(
        self,
        db: AsyncSession,
        Shipments_Cost_id: int,
        Shipments_Cost_data: Shipments_Cost,
    ) -> Shipments_Cost:
        result = await db.execute(
            select(Shipments_Cost).filter(
                Shipments_Cost.Id_Shipments_Cost == Shipments_Cost_id
            )
        )
