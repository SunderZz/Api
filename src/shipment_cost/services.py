...
async def get_all_shipment_costs_service(
    shipment_cost_repository: ShipmentsCostRepository, db: AsyncSession
...
async def get_one_shipment_service(
    shipment_id: int,
    shipment_cost_repository: ShipmentsCostRepository,
    db: AsyncSession,
) -> ShipmentsCostBase:
    shipment = await shipment_cost_repository.get_one_shipment(db, shipment_id)
    if shipment is None:
        raise HTTPException(status_code=404, detail="shipment not found not found")
    return ShipmentsCostBase(Distance=shipment.Distance, Cost=shipment.Cost)


async def create_shipment_service(
...
async def update_shipment_service(
    shipment_id: int,
    shipment: ShipmentsCostBase,
    shipment_cost_repository: ShipmentsCostRepository,
    db: AsyncSession,
