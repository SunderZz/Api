...
async def get_tva_service(tva_id: int, tva_repository: TvaRepository, db: AsyncSession) -> TvaBase:
...
async def get_tva_with_name_service(tva_name: str, tva_repository: TvaRepository, db: AsyncSession) -> TvaBase:
...
async def calculate_tva_service(tva_name: str, price: float, tva_repository: TvaRepository, db: AsyncSession) -> TvaCalculationResult:
...
async def get_all_tva_service(tva_repository: TvaRepository, db: AsyncSession) -> list[TvaBase]:
...
async def update_tva_service(tva_id: int, tva: TvaBase, tva_repository: TvaRepository, db: AsyncSession) -> TvaBase:
...
async def create_tva_service(tva: TvaCreateBase, tva_repository: TvaRepository, db: AsyncSession) -> TvaBase:
