...
async def update_user_type(
    ...
    if isFarmer and document:
        document_content = await document.read()
        producers_id = await get_producer_value(
            user_result.Id_Users, producers_repository, db
        )
        await update_producer(
            producers_id.Id_Producers,
            ProducersModifyBase(
                Id_Users=user_result.Id_Users,
                description=summary,
                Document=document_content,
            ),
            producers_repository,
        ...
        customer_id = await get_customer_value(
            user_result.Id_Users, customers_repository, db
        )
        await update_customer(
            customer_id.Id_Casual,
            CustomersUserBase(Id_Users=user_result.Id_Users),
...
async def create_new_user_address(
    ...
    db_address = await create_user_an_address(
        authorize,
        adresse,
        code_postal,
        city,
...
async def modify_user_address(
    ...
    db_address = await update_user_address(
        authorize,
        addresses_id,
        adresse,
        code_postal,
    ...
    await update_adresse_type(
        adresse_type.Id_Users_adresses, adresse_type, adresse_type_repository, db
    )
    return db_address
...
async def retrieve_user_address(
    ...
    db_address = await get_user_addresses(
...
async def create_address_type_for_user(
    ...
    db_address = await create_adresses_types(
        user_id, adresse_type, adresse_type_repository, db
    )
    return db_address


@router.put("/users/{adresse_id}/addresses", response_model=list[UsersAdressesBase])
async def update_user_adresse_type(
    adresse_id: int,
    adresse_type: AdresseTypeBase,
    user_adresse_repository: UsersAdressesRepository = Depends(UsersAdressesRepository),
    adresse_type_repository: AdresseTypesRepository = Depends(AdresseTypesRepository),
    ...
    db_address = await update_adresse_type(
        adresse_id, adresse_type, user_adresse_repository, db
    )
    return db_address
