...
class UsersAdressesRepository:
    ...
    async def create_user_address(
        self, db: AsyncSession, address: Users_adresses
    ) -> Users_adresses:
    ...
    async def update_user_address(
        self, db: AsyncSession, address_id: int, address: Users_adresses
    ) -> Users_adresses:
        result = await db.execute(
            select(Users_adresses).filter(
                Users_adresses.Id_Users_adresses == address_id
            )
        )
