...
class UsersAdressesRepository:
    ...
    async def get_user_addresses(self, db: Session, user_id: int) -> list[Users_adresses]:
        return db.query(Users_adresses).filter(Users_adresses.Id_Users_adresses == user_id).all()


    async def create_user_address(self,db: Session, address: Users_adresses) -> Users_adresses:
    ...
    async def update_user_address(self, db: Session, address_id: int, address: Users_adresses) -> Users_adresses:
