...
async def get_user_addresses(user_id: int,user_adresse_repository: UsersAdressesRepository = Depends(UsersAdressesRepository), db: Session = Depends(get_db))->list[UsersAdressesBase]:
    adresses = await user_adresse_repository.get_user_addresses(db, user_id)
    if not adresses:
...
async def create_user_address(user_id: int, address: UsersAdressesBase,user_adresse_repository: UsersAdressesRepository = Depends(UsersAdressesRepository), db: Session = Depends(get_db)) -> UsersAdressesBase:
    db_address = await user_adresse_repository.create_user_address(db, user_id, address)
